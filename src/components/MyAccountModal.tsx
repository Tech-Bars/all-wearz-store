...
const MyAccountModal:FC<props> = ({showAccountModal, setShowAccountModal}) => {
  return (
    <div className={`${showAccountModal ? "opacity-100  visible" : " opacity-0 invisible"}  transition-all delay-150 duration-300 ease-in-out shadow-lg absolute flex flex-col top-16 right-52 z-40 w-44 h-60 justify-between bg-gray-50 rounded-md`}>
       ...
       <Link  to="/account"> <h2 onClick={()=>{setShowAccountModal(!showAccountModal)}} className="text-sm text-black font-medium font-sora cursor-pointer">My Account</h2></Link>
        <h2 className="text-sm text-black font-medium font-sora cursor-pointer">My Orders</h2>
       </div>
       <div className="flex items-center p-3 justify-center border-t-2 border-primary">
        <h2 className="text-base font-medium text-primary font-sora cursor-pointer">LOGOUT</h2>
       </div>
    </div>
  )
}
